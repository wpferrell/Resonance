...
Resonance learns in two ways. First, from every message -- each call to r.process() reads the emotion, updates the user local profile, and builds a richer picture of who they are over time. Second, from explicit corrections -- if a user taps a different emotion chip to say the detection was wrong, that correction feeds directly into their per-user reinforcement loop and adjusts future detections for them. Both paths are local, no opt-in required.
...
```
New version available: pip install --upgrade resonance-layer
```
