...
# ============================================================================
# VECTOR
# 12-dimensional emotion fingerprint: [valence, arousal, dominance, +9 emotions]
# ============================================================================
...
class Storage:
    """
    Resonance storage layer.
    Both databases embedded — no server needed.

    Usage:
        store = Storage()
        record_id = store.save(result, user_id="u1", session_id="s1", topic="work")
        state = store.get_current_state("u1")
        history = store.get_recent("u1", limit=10)
        store.close()
    """
    ...
    def __init__(self):
        ...
        # Qdrant — embedded, persists to disk
        self._qdrant = QdrantClient(path=QDRANT_PATH)
        self._ensure_collection()

        # SurrealDB — async embedded
    ...
    async def _save_async(self, result, user_id, session_id, topic, timestamp):
        ...
        # 3. SurrealDB current state snapshot
    ...
    # ----------------------------------------------------------------
    # CLOSE — shuts down both databases cleanly
    # ----------------------------------------------------------------
