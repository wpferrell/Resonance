...
# Phase 5 - Live Dashboard Server
# Serves the Resonance panel with real-time WebSocket updates.
...
def _get_dashboard_data_sync():
    ...
    senses_text = senses_map.get(trend, "Here with you in whatever this is.")
    ...
    if _latest_result:
        live = {
            ...
            "valence": round(_latest_result.valence, 2),
            "arousal": round(_latest_result.arousal, 2),
        }
...
def push_update(result):
    """Called by Resonance.process() after every message. Pushes live update to panel."""
